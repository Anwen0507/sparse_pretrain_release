...
class DummyPronounTask(BinaryTask):
    ...
    # The SimpleStories CAST. The generation prompt (simple-stories/simple_stories_generate,
    # generate_stories.py) instructs: "Either don't give characters a name, or select from
    # Mia, Alex, Jean, Samuel, Lily, Leo, Jose, Kim, Alice, Lena, Rita, Emmanuel, Anne,
    # Peter, Maria or Luis" -- those 16 are the dataset's only recurring human names.
    # Pool = the 15 gender-consistent ones (each a single token, corpus freq >= 142k,
    # fp32 |she-he| gap >= 3.16, sign-consistent on every TRAIN+VAL template). "Jean" is
    # excluded: the corpus uses it for both genders (F-share 0.64; model gap +0.59), so
    # it has no valid label -- but it makes a frequency-matched no-gender-signal control.
    # Freq-ordered; stats in outputs/universality_pruning/name_pool_cast15.json.
    MALE_NAMES = ["Leo", "Alex", "Samuel", "Jose", "Emmanuel", "Peter", "Luis"]
    FEMALE_NAMES = ["Mia", "Kim", "Rita", "Lily", "Alice", "Maria", "Lena", "Anne"]
...
class DummyPronounOrig10Task(DummyPronounTask):
    """DummyPronounTask pinned to the pre-2026-06-10 name lists (5M + 5F).

    The cast15 adoption changed the base task's pool in place, so runs launched
    before it (e.g. outputs/universality_pruning/ss_d128_f1_pronoun, 2026-05-26)
    can only be repeated like-for-like through this variant. Templates inherited.
    """
    ...
    @property
    def name(self) -> str:
        return f"dummy_pronoun_orig10_{self.split}"


def gender_name_token_ids(tokenizer, as_set=True):
    """Single-token IDs of DummyPronounTask female/male names under `tokenizer`.

    Single source of truth for the gender ID sets used by the mech-interp scripts so
    they track DummyPronounTask.FEMALE_NAMES / MALE_NAMES automatically. The name lists
    now include multi-token names (validated at |she-he|>=1.0); a per-name token ID is
    only meaningful for single-token names, so multi-token names are skipped here.
    Returns (female, male) as sets by default, or order-preserving lists when as_set=False.
    """
    def single_ids(names):
        out = []
        for n in names:
            ...
            if len(ids) == 1:
                out.append(ids[0])
        return out
    female = single_ids(DummyPronounTask.FEMALE_NAMES)
    male = single_ids(DummyPronounTask.MALE_NAMES)
    return (set(female), set(male)) if as_set else (female, male)
...
class DummyTenseTask(BinaryTask):
    ...
    ALL_NAMES = ["Leo", "Alex", "Samuel", "Jose", "Peter", "Emmanuel", "Luis", "Sam", "Tim",
                 "Mia", "Kim", "Rita", "Lily", "Maria", "Alice", "Lena", "Anne"]

    # Name to pronoun mapping (for grammatically correct sentences)
    NAME_TO_PRONOUN = {
        "Leo": "he", "Alex": "he", "Samuel": "he", "Jose": "he", "Peter": "he",
        "Emmanuel": "he", "Luis": "he", "Sam": "he", "Tim": "he",
        "Mia": "she", "Kim": "she", "Rita": "she", "Lily": "she", "Maria": "she",
        "Alice": "she", "Lena": "she", "Anne": "she",
    }
...
TASK_REGISTRY = {
    ...
    "dummy_pronoun_orig10": DummyPronounOrig10Task,
